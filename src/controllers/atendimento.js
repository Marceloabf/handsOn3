...
const atendimentoController = {
  ...
  store: async (req, res) => {
    try {
      const user = req.auth
      const { paciente_id, observacao, data_atendimento } = req.body
      const newAtendimento = await Atendimento.create({
        paciente_id,
  ...
  update: async (req, res) => {
    ...
    await Atendimento.update(
      { paciente_id, psicologo_id, observacao, data_atendimento },
      {
        where: {
          id: id
        }
